...
# ================================================================
# RESNET-50 ONLY — Coral Classification
# Early Stopping | TTA | Google Drive Output
# ================================================================
...
def train_model(model, model_name='resnet50'):
    ...
    # Phase 2 — Unfreeze layer4 (epochs 11–30)
    unfreeze_layers(model, ['layer4', 'fc'])
    opt = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=5e-5, weight_decay=1e-4)
    sch = CosineAnnealingWarmRestarts(opt, T_0=20)
    if run_phase(11, 30, opt, sch, 'Phase 2 — layer4 unfrozen'):
        return early_stopper.restore_best(model), history, early_stopper.best_epoch
...
# TTA INFERENCE
# ================================================================
def predict_tta(model, image_folder):
    model.eval()
    all_preds    = []
    ground_truth = None
    for tta_tf in tta_transforms:
        ...
        with torch.no_grad():
            for imgs, _ in loader:
                ...
                tta_probs.extend(probs)
        all_preds.append(np.array(tta_probs))
        if ground_truth is None:
            ground_truth = []
            for _, lbls in DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=False):
                ground_truth.extend(lbls.numpy())
            ground_truth = np.array(ground_truth)
    return np.argmax(np.mean(all_preds, axis=0), axis=1), ground_truth
...
# ================================================================
criterion = nn.CrossEntropyLoss()
_, no_tta_acc = evaluate(resnet, test_loader, criterion)
print(f'\nResNet-50 Test Acc (no TTA): {no_tta_acc:.4f}')

print('\n── ResNet-50 + TTA ──')
preds, labels = predict_tta(resnet, TEST_DIR)
report = classification_report(labels, preds, target_names=CLASSES, digits=4)
print(report)

# Save report
report_path = '/content/classification_report_resnet50.txt'
with open(report_path, 'w') as f:
    f.write(f'ResNet-50 Test Acc (no TTA): {no_tta_acc:.4f}\n\n')
    f.write('ResNet-50 + TTA Classification Report:\n')
    f.write(report)
save_to_drive(report_path)

# Confusion matrix
plot_confusion_matrix(labels, preds, 'ResNet50 TTA')
